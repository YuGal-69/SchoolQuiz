 ...
 */
export const createPoll = (pollData, teacherSocketId) => {
...
export const startPoll = (io) => {
  if (state.poll && !state.poll.isActive) {
    ...
    console.log(`Poll ${state.poll.id} started.`);
    return state.poll;
  }
  return null;
};
...
export const endPoll = (io) => {
  if (state.poll && state.poll.isActive) {
    ...
    console.log(`Poll ${state.poll.id} ended.`);
...
export const addParticipant = (socketId, name) => {
  state.participants[socketId] = { id: socketId, name };
...
export const removeParticipant = (socketId) => {
  if (state.participants[socketId]) {
    console.log(`Participant removed: ${state.participants[socketId].name}`);
...
export const allStudentsVoted = () => {
  ...
  const studentCount = Object.values(state.participants).filter(
    (p) => p.id !== state.poll.teacherSocketId
  ).length;
